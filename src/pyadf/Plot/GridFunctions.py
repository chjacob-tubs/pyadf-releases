...
class GridFunction(object):
    ...
    def apply_function(self, func):
        ...
        if isinstance(elem, numpy.ndarray):
            new_values = numpy.empty((self.values.shape[0],) + elem.shape)
        else:
            new_values = numpy.empty((self.values.shape[0],))
...
class GridFunction1D(GridFunction):
    ...
    def filter_negative(self, thresh=0.0):
        ...
        new_values = numpy.minimum(self.values, [thresh])

        # calculate checksum for negative density
        import hashlib
        m = hashlib.md5()
        m.update( \
          ("Grid function keeping only negative values, thresh %18.8f :\n" % thresh).encode("utf-8"))
        m.update(self.get_checksum())
    ...
    def filter_positive(self, thresh=0.0):
        ...
        new_values = numpy.maximum(self.values, [thresh])

        # calculate checksum for negative density
        import hashlib
        m = hashlib.md5()
        m.update( \
          ("Grid function keeping only positve values, thresh %18.8f :\n" % thresh).encode("utf-8"))
        m.update(self.get_checksum())
    ...
    def filter_zeros(self, thresh=1e-4):
        ...
        new_values = numpy.where(numpy.logical_and(self.values >= 0.0, self.values < thresh),
                                 +thresh, self._values)

        new_values = numpy.where(numpy.logical_and(new_values < 0.0, new_values > -thresh),
                                 -thresh, new_values)
        ...
        m = hashlib.md5()
        m.update( \
          ("Grid function obtained from filter_zeros, thresh %18.8f :\n" % thresh).encode("utf-8"))
        m.update(self.get_checksum())
    ...
    def _add_constant(self, const):
        ...
        m.update("Grid function obtained by adding :\n".encode("utf-8"))
        m.update(self.get_checksum())
        m.update(("Constant shift %18.10f" % const).encode("utf-8"))
    ...
    def integral(self, func=None, ignore=None, involume=None):
        ...
        if involume:
            ...
            if ignore is None:
                for w, val in zip(self.grid.weightiter(), self.valueiter()):
                    ii += w * func(val)
            else:
                for w, val, ig in zip(self.grid.weightiter(), self.valueiter(), ignore.flat):
                    if not ig:
                        ii += w * func(val)
        return ii
...
class GridFunction2D(GridFunction):
    ...
    def abssquare(self):
        ...
        new_values = numpy.empty((self.values.shape[0],))
        for i in range(self.values.shape[0]):
            new_values[i] = numpy.dot(self.values[i, :], self.values[i, :])
...
class GridFunctionDensity(GridFunction1D):
    ...
    def get_electronic_dipole_voronoi(self, atoms):
        ...
        lvoronoidip = []
        for iatom in range(len(atoms)):
            lvoronoidip.append(numpy.array(voronoidip[iatom]))
...
class GridFunctionContainer(object, metaclass=_GridFunctionContainerMetaclass):
...
class GridFunctionUnrestricted(GridFunctionContainer):
    ...
    @property
    def values(self):
        vals = numpy.empty(self.wrapped[0].values.shape + (2,))
...
class GridFunctionFactory(object):
    ...
    def newGridFunctionUnrestricted(cls, grid, values_alpha, values_beta,
        ...
        if checksum is not None:
            import hashlib
            ma = hashlib.md5()
            ma.update("Alpha grid function:".encode("utf-8"))
            ma.update(checksum)
            checksum_a = ma.digest()

            mb = hashlib.md5()
            mb.update("Beta grid function:".encode("utf-8"))
            mb.update(checksum)
