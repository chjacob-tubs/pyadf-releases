...
import os
import numpy as np
from pyadf.PyEmbed.Plot.FileReaders import GridReader
...
class TestGridReader(PyAdfTestCase):
    ...
    def test_read_xvzw(self):
        ...
        saved_weights = np.load(os.path.join(TESTDATA_DIRNAME, 'grid-xyzv-weights.npy'))
        self.assertAlmostEqualNumpy(grid.weights, saved_weights, 8)
        saved_coords = np.load(os.path.join(TESTDATA_DIRNAME, 'grid-xyzv-coords.npy'))
    ...
    def test_read_xml(self):
        ...
        saved_weights = np.load(os.path.join(TESTDATA_DIRNAME, 'grid-xml-weights.npy'))
        self.assertAlmostEqualNumpy(grid.weights, saved_weights, 8)
        saved_coords = np.load(os.path.join(TESTDATA_DIRNAME, 'grid-xml-coords.npy'))
        self.assertAlmostEqualNumpy(grid.get_coordinates(), saved_coords, 8)
...
class TestGridFunctionReader(PyAdfTestCase):
    ...
    def test_read_xvzwv(self):
        ...
        saved_values = np.load(os.path.join(TESTDATA_DIRNAME, 'grid-xyzv-values.npy'))
    ...
    def test_read_density_elpot_xyzwv(self):
        ...
        saved_pot = np.load(os.path.join(TESTDATA_DIRNAME, 'density-pot.npy'))
        self.assertAlmostEqualNumpy(pot_gf.get_values(), saved_pot, 8)
        saved_rho = np.load(os.path.join(TESTDATA_DIRNAME, 'density-rho.npy'))
        self.assertAlmostEqualNumpy(rho_gf[0].get_values(), saved_rho, 8)
        saved_rhod = np.load(os.path.join(TESTDATA_DIRNAME, 'density-rhod.npy'))
    ...
    def test_read_density_elpot_xml(self):
        ...
        saved_pot = np.load(os.path.join(TESTDATA_DIRNAME, 'density-xml-elpot.npy'))
        self.assertAlmostEqualNumpy(elpot_gf.get_values(), saved_pot, 8)
        saved_pot = np.load(os.path.join(TESTDATA_DIRNAME, 'density-xml-nucpot.npy'))
        self.assertAlmostEqualNumpy(nucpot_gf.get_values(), saved_pot, 8)

        saved_rho = np.load(os.path.join(TESTDATA_DIRNAME, 'density-xml-rho.npy'))
        self.assertAlmostEqualNumpy(rho_gf[0].get_values(), saved_rho, 8)
        saved_rhod = np.load(os.path.join(TESTDATA_DIRNAME, 'density-xml-rhod.npy'))
        self.assertAlmostEqualNumpy(rho_gf[1].get_values(), saved_rhod, 8)
        saved_rhodd = np.load(os.path.join(TESTDATA_DIRNAME, 'density-xml-rhodd.npy'))
