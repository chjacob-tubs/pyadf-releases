...
class Grid:
    ...
    def get_discrete_laplacian(self):
        ...
        w = scipy.sparse.diags([np.sqrt(self.w2)], [0], format='csc')
...
def eigh_davidson(A, k, startvecs, convmax=1e-7, convnorm=1e-12):
    ...
    while (maxres > convmax) and (resnorm > convnorm) and (it < n):
        ...
        # orthogonalize bvecs
        bvecs, dum = np.linalg.qr(bvecs)

        # construct subspace Hamiltonian
        sigma_sub = A.dot(bvecs)
        A_sub = np.dot(bvecs.transpose(), sigma_sub)

        # diagonalize subspace Hamiltonian
        evals, evecs_sub = np.linalg.eigh(A_sub)
        ...
        bvecs = np.dot(bvecs, evecs_sub)

        # calculate residue vectors
        resid = np.zeros((n, k))
        for i in range(k):
            resid[:, i] = np.dot(sigma_sub, evecs_sub[:, i]) - evals[i] * bvecs[:, i]
        maxres = np.max(np.abs(resid))
        ...
        bvecs = np.concatenate((bvecs, b_new), axis=1)
...
class OneDimSolver:
    ...
    def calc_orbitals(self, pot, ll, norbs, kmat=None):
        ...
        for i in range(norbs):
            ...
            evecs[:, i] = np.sqrt(self.grid.w2) * evecs[:, i]
            # normalize P
    ...
    def calc_density(self, pot, occs, ons=None, output=True):
        ...
        dens = np.zeros_like(self.grid.r)
...
class CalcFuncGrad:
    ...
    def hess(self, pot, lambda_smooth=None):
        H = np.zeros((self.grid.N, self.grid.N))
        ...
        for ll in range(maxl + 1):
            ...
            Hl = np.zeros((self.grid.N, self.grid.N))
            ...
            for iocc in range(self.occs[ll]):
                ...
                for ivirt in range(self.grid.N):
                    if not (iocc == ivirt):
                        ...
                        Hl = Hl + self.ons[ll][iocc] * np.outer(prodorbs, prodorbs) / (ens[iocc] - ens[ivirt])
        ...
        H = np.outer(1.0 / (self.grid.r * self.grid.w2), 1.0 / (self.grid.r * self.grid.w2)) * H
    ...
    def invhess(self, pot):
        ...
        evals = np.abs(1.0 / (evals + 1e-8))  # cutoff small eigenvalues here
        Hinv = np.dot(evecs, np.dot(np.diag(evals), evecs.transpose()))
    ...
    def error(self):
        abserr = 4.0 * math.pi * self.grid.calc_integral(np.abs(self.dens - self.refdens))
        maxdens = np.max(np.abs(self.dens - self.refdens) / (self.grid.r * self.grid.r))
    ...
    def info(self):
        ...
        gradnorm = np.linalg.norm(self.grad)
...
class CalcFuncGradSpin:
    ...
    def gradient(self, pot):
        ...
        self.grad = np.concatenate([grad_alpha, grad_beta])
        return self.grad
    ...
    def hess(self, pot):
        ...
        H = np.vstack((np.hstack((Haa, Hab)), np.hstack((Hba, Hbb))))
        return H
...
def reconstruct_spinpotential_sd(grid_in, refdens_alpha_in, refdens_beta_in, startpot_in, occs_alpha_in, occs_beta_in,
    ...
    startpot = np.concatenate([startpot_in * grid_in.r, startpot_in * grid_in.r])
