...
from PlotGrids     import cubegrid, adfgrid
from Plot          import densfjob, densfresults, densfresults_unrestricted
...
class adfsettings (object):
    ...
    def set_functional (self, functional) :
        ...
        if functional in ['LDA', 'HARTREEFOCK'] :
...
class adfscfsettings(object):
    ...
    def __init__ (self, basis, settings, core=None, pointcharges=None, options=None, createproc=None, frozenfrags=None) :
        """
...
class adfsinglepointresults (adfresults) :
    ...
    def get_nonfrozen_density (self, grid=None, spacing=0.5, fit=False) :
        ...
        """
        return self.get_density (grid=grid, spacing=spacing, fit=fit)
    ...
    def get_density (self, grid=None, spacing=0.5, fit=False) :
        ...
        if self.get_nspin() == 2:
            alpha = densfjob(self, prop + " alpha", grid=grid).run()
            beta  = densfjob(self, prop + " beta", grid=grid).run()
            return densfresults_unrestricted(alpha, beta)
        else:
            return densfjob(self, prop, grid=grid).run()

    def get_orbital(self, grid=None, spacing=0.5, irrep=None, num=None):
        '''
        ...
        return densfjob(self, 'Orbital %s %i'%(irrep, num), grid=grid).run() 
        
    def get_orbital_laplacian(self, grid=None, spacing=0.5, irrep=None, num=None):
        '''
        ...
        return densfjob(self, 'OrbLapl %s %i'%(irrep, num), grid=grid).run() 
        
    def get_orbital_density(self, grid=None, spacing=0.5, orbs=None):
        '''
        ...
        if orbs.keys() == ['A'] :
            orbdens = densfjob (self, 'OrbDens %s'%(repr(orbs_temp['A'])), grid=grid).run() 
        else :        
            orbdens = []
    ...
    def get_locorb_density(self, grid=None, spacing=0.5, orbs=None):
        '''
        Get the density of a given set of localized orbitals.
        ...
        return densfjob (self, 'LocOrbDens %s'%(repr(orbs)), grid=grid).run()

    def get_sqrgradient (self, grid=None) :
        """
    ...
    def get_locorb_sqrgradient(self, grid=None, spacing=0.5, orbs=None):
        '''
        Get the squared density gradient for a given set of localized orbitals.
        ...
        return densfjob (self, 'LocOrbGrad %s'%(repr(orbs)), grid=grid).run()

    def get_orbdens_sqrgradient(self, grid=None, spacing=0.5, orbs=None):
        '''
        Get the squared density gradient for a given set of localized orbitals.
        ...
        if not (orbs.keys() == ['A']) :
            raise PyAdfError('get_orbdens_sqrgradient only working for NSYM (irrep A) orbitals')

        return densfjob (self, 'OrbDensGrad %s'%(repr(orbs_temp['A'])), grid=grid).run()

    def get_orbdens_gradient(self, grid=None, spacing=0.5, orbs=None):
        '''
        Get the squared density gradient for a given set of localized orbitals.
        ...
        if not (orbs.keys() == ['A']) :
            raise PyAdfError('get_orbdens_gradient only working for NSYM (irrep A) orbitals')

        return densfjob (self, 'OrbDensGradX %s'%(repr(orbs_temp['A'])), grid=grid).run(), \
               densfjob (self, 'OrbDensGradY %s'%(repr(orbs_temp['A'])), grid=grid).run(), \
               densfjob (self, 'OrbDensGradZ %s'%(repr(orbs_temp['A'])), grid=grid).run()

    def get_laplacian (self, grid=None, fit=False) :
        """
        ...
        if self.get_nspin() == 2:
            if fit :
                alpha = densfjob(self, prop + " alpha Fit", grid=grid).run()
                beta  = densfjob(self, prop + " beta Fit", grid=grid).run()
            else:
                alpha = densfjob(self, prop + " alpha", grid=grid).run()
                beta  = densfjob(self, prop + " beta", grid=grid).run()
            return densfresults_unrestricted(alpha, beta)
    ...
    def get_orbdens_laplacian (self, grid=None, spacing=0.5, orbs=None) :
        """
        Returns the Laplacian of the electron density for selected orbitals.
        ...
        if not (orbs.keys() == ['A']) :
            raise PyAdfError('get_orbdens_laplacian only working for NSYM (irrep A) orbitals')

        return densfjob (self, 'OrbDensLapl %s'%(repr(orbs_temp['A'])), grid=grid).run()
    ...
    def get_potential (self, grid=None, pot='total'):
        ...
        @rtype: L{densfresults}
        """
        prop = 'Potential '+pot.lower()
        if (self.get_nspin() == 2) and not (pot.lower() in ['nuc', 'coul']):
            alpha = densfjob(self, prop + " alpha", grid=grid).run()
            beta  = densfjob(self, prop + " beta", grid=grid).run()
            return densfresults_unrestricted(alpha, beta)
        else:
    ...
    def get_kinetic_potential (self, grid=None, func='THOMASFERMI'):
        """
        ...
        """
        if self.get_nspin() == 2 :
            alpha = densfjob(self, 'KINPOT '+func.upper()+' ALPHA', grid=grid).run()
            beta  = densfjob(self, 'KINPOT '+func.upper()+' BETA', grid=grid).run()
            return densfresults_unrestricted(alpha, beta)
        else:
            return densfjob(self, 'KINPOT '+func.upper(), grid=grid).run()

    def get_locorb_kinpot(self, grid=None, orbs=None, func='THOMASFERMI'):
        '''
        Get the kinetic potential for the density of localized orbitals.
        ...
        return densfjob (self, 'LocOrbKinPot %s %s'%(func.upper(), repr(orbs)), grid=grid).run()
    ...
    def get_orbdens_kinpot(self, grid=None, orbs=None, func='THOMASFERMI'):
        ...
        if not (orbs.keys() == ['A']) :
            raise PyAdfError('get_orbdens_kinpot only working for NSYM (irrep A) orbitals')

        return densfjob (self, 'OrbDensKinPot %s %s' % (func.upper(), repr(orbs_temp['A'])), grid=grid).run()
    ...
    def get_locorb_xcpot(self, grid=None, orbs=None):
        ...
        @rtype: L{densfresults}
        '''
        return densfjob (self, 'LocOrbXCPot %s'%(repr(orbs)), grid=grid).run()

    def get_locorb_coulpot(self, grid=None, orbs=None):
        '''
        Get the Coulomb potential for the density of localized orbitals.

        This is calculated using density fitting in CJDENSF.
        ...
        @rtype: L{densfresults}
        '''
        return densfjob (self, 'LocOrbCoulPot %s'%(repr(orbs)), grid=grid).run()
    ...
    def get_locorb_coulpot_numint(self, grid=None, orbs=None):
        ...
        dens = self.get_locorb_density(grid=self.grid, orbs=orbs)
        
        # get ADF grid weights and coordinates and prepare arrays
        
        weights = numpy.zeros((self.grid.get_number_of_points(),))
        coords  = numpy.zeros((self.grid.get_number_of_points(), 3))
        ...
        coulpot = numpy.zeros(grid.get_number_of_points())
        p = numpy.empty_like(coords)

        print "Number of points to calculate: ", grid.get_number_of_points()
        
        for i, point in enumerate(grid.coorditer()) :
            if i % 500 == 0 :
                print "Calculating potential for point %i of %i " % (i, grid.get_number_of_points())
        ...
        # finally create a densfresults object
        res = densfresults(grid=grid)

        res._values = coulpot
        #pylint: disable-msg=W0212
        res._values.shape = res.grid.shape
        ...
        res._checksum = m.digest()

        return res

    def get_final_potential (self):
        """
        Get the final total potential from the jos's TAPE10 file.
        ...
        @rtype: L{densfresults}
        """
        pot = densfresults(None, grid=self.grid)
        pot.files = self.files 
        pot.prop  = 'potential total'
        ...
        m.update(self.get_checksum())
        pot._checksum = m.digest()

        pot._values = self.get_result_from_tape('Total Potential', 'vtot', tape=10) 
    ...
    def export_grid (self, filename):
        ...
        for iblock in range(1, nblock+1) :
            ...
            for c, w in zip(coords, weights) :
                outfile.write("%25.18e  %25.18e  %25.18e  %25.18e \n" % (c[0], c[1], c[2], w))
