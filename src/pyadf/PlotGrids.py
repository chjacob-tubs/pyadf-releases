...
class grid (object):
    ...
    @undocumented:
        __copy__, __deepcopy__, _get_shape
    """
    ...
    def _get_mol (self):
        return None
    mol = property(_get_mol, None, None)
    ...
    def write_grid_to_t41 (self, iu):
        ...
        iu.writeints('Grid', 'total nr of points', self.get_number_of_points())
        ...
        coords = coords/Bohr_in_Angstrom
    ...
    def get_coordinates (self, bohr=False):
        ...
        """
        import numpy
        coords = numpy.zeros((self.get_number_of_points(), 3))
...
class cubegrid (grid):
    ...
    def write_grid_to_t41 (self, iu):
        ...
        iu.writereals('Grid', 'Start_point', numpy.array(self._startpoint)/Bohr_in_Angstrom)
        ...
        iu.writereals('Grid', 'x-vector', [self._spacing/Bohr_in_Angstrom, 0.0, 0.0])
        iu.writereals('Grid', 'y-vector', [0.0, self._spacing/Bohr_in_Angstrom, 0.0])
        iu.writereals('Grid', 'z-vector', [0.0, 0.0, self._spacing/Bohr_in_Angstrom])
    ...
    def get_cube_header (self):
        ...
        header += " %5d %12.6f %12.6f %12.6f \n" % (self._mol.get_number_of_atoms(), 
                                                    self._startpoint[0] / Bohr_in_Angstrom,
                                                    self._startpoint[1] / Bohr_in_Angstrom,
                                                    self._startpoint[2] / Bohr_in_Angstrom)
        # lines 4-6: no grid points in x,y,z direction + unit vector
        header += " %5d %12.6f %12.6f %12.6f \n" % \
                  (self._npoints[0], self._spacing / Bohr_in_Angstrom, 0.0, 0.0)
        header += " %5d %12.6f %12.6f %12.6f \n" % \
                  (self._npoints[1], 0.0, self._spacing / Bohr_in_Angstrom, 0.0)
        header += " %5d %12.6f %12.6f %12.6f \n" % \
                  (self._npoints[2], 0.0, 0.0, self._spacing / Bohr_in_Angstrom)
    ...
    def get_xsf_header (self):
        ...
        header += " %12.6f %12.6f %12.6f \n" % ( self._startpoint[0] / Bohr_in_Angstrom,
                                                 self._startpoint[1] / Bohr_in_Angstrom,
                                                 self._startpoint[2] / Bohr_in_Angstrom)
        header += " %12.6f %12.6f %12.6f \n" % (self._spacing / Bohr_in_Angstrom, 0.0, 0.0)
        header += " %12.6f %12.6f %12.6f \n" % (0.0, self._spacing / Bohr_in_Angstrom, 0.0)
        header += " %12.6f %12.6f %12.6f \n" % (0.0, 0.0, self._spacing / Bohr_in_Angstrom)
    ...
    def weightiter (self):
        import itertools
        return itertools.repeat (pow(self._spacing/Bohr_in_Angstrom, 3) ,
                                 self._npoints[0]*self._npoints[1]*self._npoints[2])
...
class adfgrid (grid):
    ...
    def get_grid_block (self, checksumonly):
        ...
        if checksumonly :
            block += self._adfres.get_checksum()
        return block
    ...
    def voronoiiter(self):
        ...
        symbols = self.mol.get_atom_symbols()
        coords = numpy.array(self.mol.get_coordinates())

        atomnumbers = []
        for i in range(1, natoms+1) :
            a = self.mol.mol.GetAtom(i)
...
class customgrid (grid) :
    ...
    def write_grid_tape10 (self, name='TAPE10'):
        ...
        lblock = 128
        if (self._npoints % lblock == 0) :
            dummypoints = 0
        else :
            dummypoints = lblock - (self._npoints % lblock)
        npoints_total = self._npoints + dummypoints
        ...
        data[:self._npoints, :3] = self._coords
        if self._weights is None :
            data[:self._npoints, 3]  = 1.0
        else :
            data[:self._npoints, 3]  = self._weights
    ...
    def weightiter (self):
        if self._weights is None :
            raise PyAdfError('No weights present in customgrid')
        else :
            for i in range(self._npoints) :
...
class interpolation (object):
    ...
    def __init__ (self, dens):
        ...
        @param dens: The density/potential to be interpolated
        @type  dens: L{densfresults} 
        '''
        self.v = dens.get_values()
        self.z = dens.grid.get_coordinates() 
        self.mol = dens.grid.mol
