    ...
    *blocksize* indicates the length of basic KF file block. So far, all KF files produced by 
    ...
    def __init__(self, path, blocksize=4096, autodetect=True):
        if os.path.isfile(path):
            ...
            raise KFFileError('File {} not present'.format(path))
    ...
    def read(self, section, variable):
        ...
        if vtype == 3:
            ...
            if (vtype == 1) and (self.word == 'i'):
                ret = numpy.empty(vlen, dtype=numpy.int32)
            elif (vtype == 1) and (self.word == 'q'):
                ret = numpy.empty(vlen, dtype=numpy.int64)
            elif vtype == 2:
                ret = numpy.empty(vlen, dtype=numpy.float_)
            elif vtype == 4:
                ret = numpy.empty(vlen, dtype=numpy.bool)
    ...
    def _get_data(self, datablock, vtype):
        ...
        contents = self._parse_one_chunk(datablock[hlen:], zip((i, d, s, b), (self.word, 'd', 's', self.word)))
        if vtype == 1:
            return contents[:i]
        elif vtype == 2:
            ...
            return map(bool, contents[i + d + 1:])
        else:
...
class KFFile:
    ...
    def read(self, section, variable, return_as_list=False):
        ...
        if return_as_list and not isinstance(ret, (list, numpy.ndarray)):
            ret = [ret]
    ...
    def delete_section(self, section):
        ...
        if self.reader:
            ...
            if section in self.reader._sections:
                ...
                subprocess.run(['cpkf', self.path, tmpfile, '-rm', section], stdout=DEVNULL, stderr=DEVNULL)
