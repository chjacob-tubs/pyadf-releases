...
from pyadf.Plot.GridFunctions import GridFunction1D
from pyadf.Plot.GridFunctions import GridFunction2D
from pyadf.Plot.GridFunctions import GridFunctionDensity
from pyadf.Plot.GridFunctions import GridFunctionPotential
...
class TestGridFunction(PyAdfTestCase):
    ...
    def setUp(self):
        ...
        values1 = numpy.array([[[1., 2.], [3., 4.]],
                               [[5., 6.], [7., 8.]],
                               [[2., 2.], [3., 3.]]])
        self.gf1 = GridFunction(grid, values1)

        values2 = numpy.array([[[2., 2.], [2., 2.]],
                               [[1., 1.], [1., 1.]],
                               [[1., 2.], [3., 4.]]])
        self.gf2 = GridFunction(grid, values2)

    def test_add(self):
        ref_values = numpy.array([[[3., 4.], [5., 6.]],
                                  [[6., 7.], [8., 9.]],
                                  [[3., 4.], [6., 7.]]])
    ...
    def test_sub(self):
        ref_values = numpy.array([[[-1., 0.], [1., 2.]],
                                  [[4., 5.], [6., 7.]],
                                  [[1., 0.], [0., -1.]]])
    ...
    def test_mul_scalar(self):
        ref_values = numpy.array([[[2., 4.], [6., 8.]],
                                  [[10., 12.], [14., 16.]],
                                  [[4., 4.], [6., 6.]]])
        prod = self.gf1 * 2.0
    ...
    def test_negative(self):
        ref_values = numpy.array([[[-1., -2.], [-3., -4.]],
                                  [[-5., -6.], [-7., -8.]],
                                  [[-2., -2.], [-3., -3.]]])
        res = -self.gf1
    ...
    def test_div_scalar(self):
        ref_values = numpy.array([[[0.5, 1.], [1.5, 2.]],
                                  [[2.5, 3.], [3.5, 4.]],
                                  [[1., 1.], [1.5, 1.5]]])
        qout = self.gf1 / 2.0
    ...
    def test_apply_function(self):
        ref_values = numpy.array([11., 83., 12.])
        res = self.gf1.apply_function(lambda x: numpy.dot(x[0], x[1]))
        self.assertAlmostEqual(res.values, ref_values)
...
class TestGridFunction1D(PyAdfTestCase):
    ...
    def setUp(self):
        ...
        values1 = numpy.array([1., 2., 3.])
        self.gf1 = GridFunction1D(grid, values1)

        values2 = numpy.array([2., 2., 6.])
        self.gf2 = GridFunction1D(grid, values2)

        values3 = numpy.array([-1., 0., 1.])
        self.gf3 = GridFunction1D(grid, values3)

        values4 = numpy.array([-0.001, 0.001, 0.1])
        self.gf4 = GridFunction1D(grid, values4)

    def test_add(self):
        ref_values = numpy.array([3., 4., 9.])
        summ = self.gf1 + self.gf2
        self.assertAlmostEqual(summ.values, ref_values)

    def test_sub(self):
        ref_values = numpy.array([1., 0., 3.])
        diff = self.gf2 - self.gf1
        self.assertAlmostEqual(diff.values, ref_values)

    def test_add_constant(self):
        ref_values = numpy.array([4., 5., 6.])
        summ = self.gf1 + 3.0
        self.assertAlmostEqual(summ.values, ref_values)
    ...
    def test_filter_volume(self):
        ref_values = numpy.array([1., 0., 0.])
        res = self.gf1.filter_volume(lambda x: x[0] + x[1] + x[2] < 2.0)
        self.assertAlmostEqual(res.values, ref_values)

    def test_filter_positive(self):
        ref_values = numpy.array([0., 0., 1.])
        res = self.gf3.filter_positive()
        self.assertAlmostEqual(res.values, ref_values)

        ref_values = numpy.array([2., 2., 3.])
    ...
    def test_integral(self):
        ...
        integral = self.gf1.integral(ignore=numpy.array([False, False, True]))
        self.assertAlmostEqual(integral, 2.0)

        integral = self.gf1.integral(func=lambda x: x * x, ignore=numpy.array([True, False, False]))
...
class TestGridFunctionFactory(PyAdfTestCase):
    ...
    def setUp(self):
        ...
        self.values_1d = numpy.array([1., 2., 3.])
        self.values_2d = numpy.array([[1., 2., 3.], [1., 1., 1.], [4., 5., 6.]])
        self.values_3d = numpy.array([[[1., 2.], [3., 4.]],
                                      [[5., 6.], [7., 8.]],
                                      [[2., 2.], [3., 3.]]])
...
class TestGridFunctionOperatorsResultTypes(PyAdfTestCase):
    ...
    def setUp(self):
        ...
        values_1d = numpy.array([1., 2., 3.])
        values_2d = numpy.array([[1., 2., 3.], [1., 1., 1.], [4., 5., 6.]])
        values_3d = numpy.array([[[1., 2.], [3., 4.]],
                                 [[5., 6.], [7., 8.]],
                                 [[2., 2.], [3., 3.]]])
...
class TestGridFunctionContainer(PyAdfTestCase):
    ...
    def test_init_getitem(self):
        ref_values_1a = numpy.array([1., 2., 3.])
        ref_values_1b = numpy.array([4., 5., 6.])
    ...
    def test_add_constant(self):
        ...
        ref_values_a = numpy.array([2., 3., 4.])
        ref_values_b = numpy.array([5., 6., 7.])
    ...
    def test_add(self):
        ...
        ref_values_a = numpy.array([3., 4., 5.])
        ref_values_b = numpy.array([7., 9., 3.])
        self.assertAlmostEqual(res_container[0].values, ref_values_a)
        self.assertAlmostEqual(res_container[1].values, ref_values_b)
    ...
    def test_sub_constant(self):
        ...
        ref_values_a = numpy.array([0., 1., 2.])
        ref_values_b = numpy.array([3., 4., 5.])
        self.assertAlmostEqual(res_container[0].values, ref_values_a)
        self.assertAlmostEqual(res_container[1].values, ref_values_b)
    ...
    def test_sub(self):
        ...
        ref_values_a = numpy.array([-1., 0., 1.])
    ...
    def test_mul_scalar(self):
        ...
        ref_values_a = numpy.array([2., 4., 6.])
        ref_values_b = numpy.array([8., 10., 12.])
        self.assertAlmostEqual(res_container[0].values, ref_values_a)
        self.assertAlmostEqual(res_container[1].values, ref_values_b)
    ...
    def test_mul(self):
        ...
        ref_values_a = numpy.array([2., 4., 6.])
        ref_values_b = numpy.array([12., 20., -18.])
        self.assertAlmostEqual(res_container[0].values, ref_values_a)
        self.assertAlmostEqual(res_container[1].values, ref_values_b)
    ...
    def test_div_scalar(self):
        ...
        ref_values_a = numpy.array([0.25, .5, 0.75])
        ref_values_b = numpy.array([1., 1.25, 1.5])
        self.assertAlmostEqual(res_container[0].values, ref_values_a)
        self.assertAlmostEqual(res_container[1].values, ref_values_b)
    ...
    def test_filter_positive(self):
        ...
        ref_values_a = numpy.array([2., 2., 2.])
        ref_values_b = numpy.array([3., 4., 0.])
        self.assertAlmostEqual(res_container[0].values, ref_values_a)
        self.assertAlmostEqual(res_container[1].values, ref_values_b)
    ...
    def test_filter_negative(self):
        ...
        ref_values_a = numpy.array([0., 0., 0.])
        ref_values_b = numpy.array([0., 0., -3.])
        self.assertAlmostEqual(res_container[0].values, ref_values_a)
        self.assertAlmostEqual(res_container[1].values, ref_values_b)
    ...
    def test_filter_zeros(self):
        ...
        ref_values_a = numpy.array([3.5, 3.5, 3.5])
        ref_values_b = numpy.array([3.5, 4., -3.5])
        self.assertAlmostEqual(res_container[0].values, ref_values_a)
        self.assertAlmostEqual(res_container[1].values, ref_values_b)
    ...
    def test_add_gf(self):
        ...
        ref_values_a = numpy.array([4., 6., 0.])
        ref_values_b = numpy.array([7., 9., 3.])
        self.assertAlmostEqual(res_container[0].values, ref_values_a)
        self.assertAlmostEqual(res_container[1].values, ref_values_b)
    ...
    def test_sub_gf(self):
        ...
        ref_values_a = numpy.array([-2., -2., 6.])
        ref_values_b = numpy.array([1., 1., 9.])
        self.assertAlmostEqual(res_container[0].values, ref_values_a)
        self.assertAlmostEqual(res_container[1].values, ref_values_b)
